var EBE_Combox = function(el,emptyVal,index){
    this.el = el;
...
(function(){
    this.init = function(){
        ...
        this.el.change(function(){
            that.index = $.inArray(that.el.val(), that.data );
            if( that.changeHandler ){
                that.changeHandler( that.index );
            }
        });
    };
    this.setData = function(data){
        ...
        for( i=0; i < data.length;i++ ){
            opt = this.getOpt(i);
            this.useds.push( opt );
            opt.val( data[i] ).text(data[i]).appendTo(this.el);
        }
        this.index = -1;
        this.emptyOpt.prop("selected",true);
    };
    this.setChangeHandler = function(fn){
...
var EBE_AddressSelector = function(){
    var emptyValue= ["--请选择省份--","--请选择城市--","--请选择地区--"];
    ...
    var selectEls = $(".appendAddressBlock select");
    var i,k,z,str,strA,arrA,arrB,arrT1,arrT2,indexA,indexB,provincesData = PCAD.split("#"),citys,zone;
    var comboxs = [];
    for( i=0; i < selectEls.length;i++){
        comboxs.push( new EBE_Combox( selectEls.eq(i),emptyValue[i],i  ) );
    }
    var pData = [],cData = [], aData = [];
    var pIndex=-1,cIndex=-1,aIndex=-1;

    for(i=0; i < provincesData.length;i++){
        str =  provincesData[i];
        indexA = str.indexOf("$");
        strA = str.substring(0, indexA);
        pData.push(strA);

        arrA = str.substring(indexA+1).split("|");
        arrT1= [];
        arrT2= [];
    ...
    comboxs[0].setData(pData);
    comboxs[0].setChangeHandler(function(index){
        comboxs[1].setData( cData[index]  );
        comboxs[2].setData([]);
    });
    comboxs[1].setChangeHandler(function(index){
        comboxs[2].setData( aData[comboxs[0].getIndex()][index]   );
    });
};
...
var EBE_FormManager = function(){
    ...
    var addressRowEl = formEl.find(".addressList");
    var addressRadioEls = addressRowEl.find("input");
    var appendAddressBlockEl = formEl.find(".appendAddressBlock");
    addressRadioEls.change(addressInputChangeHandler);
    function addressInputChangeHandler(){
        var radioEl = addressRowEl.find("input:checked");
        if( radioEl.length==1 && radioEl.val()=="" ){
            appendAddressBlockEl.show();
        }else{
            appendAddressBlockEl.hide();
        }
    }
    addressInputChangeHandler();
    var stepNavEl =$(".mainPanel .processPanel ");

    var recommendRowEl = formEl.find(".recommendRow");
    var recommendRadioEl = recommendRowEl.find("input[type='radio']");
    var recommendInputBlockEl = recommendRowEl.find(".inputBorder");
    var recommendInputEl = recommendInputBlockEl.find("input");
    var recommendHintEl = recommendInputBlockEl.find("b");
    var recommendIndex = 0;
    recommendRadioEl.change(function(){
        var tIndex = recommendRadioEl.index(this);
        recommendIndex = tIndex;
        if(recommendIndex==0){
            recommendInputBlockEl.css("visibility","hidden");
        }else{
            recommendInputBlockEl.css("visibility","visible");
        }
    });
    recommendInputEl.focus(function(){
        recommendHintEl.hide();
    }).blur(function(){
        if( $.trim( recommendInputEl.val() ) == ""){
            recommendInputEl.val("");
            recommendHintEl.show();
        }
    });

    var nameBorderEl =  appendAddressBlockEl.find(".inputBorder:eq(0)");
    var nameInputEl =  appendAddressBlockEl.find("input:eq(0)");

    var provincesBorderEl =  appendAddressBlockEl.find(".inputBorder:eq(1)");
    var provincesInputEl =  appendAddressBlockEl.find("select:eq(0)");
    var cityBorderEl =  appendAddressBlockEl.find(".inputBorder:eq(2)");
    var cityInputEl =  appendAddressBlockEl.find("select:eq(1)");
    var areaBorderEl =  appendAddressBlockEl.find(".inputBorder:eq(3)");
    var areaInputEl =  appendAddressBlockEl.find("select:eq(2)");

    var addressBorderEl = appendAddressBlockEl.find(".inputBorder:eq(4)");
    var addressInputEl = appendAddressBlockEl.find("input:eq(1)");

    var phoneBorderEl = appendAddressBlockEl.find(".inputBorder:eq(5)");
    var phoneInputEl = appendAddressBlockEl.find("input:eq(2)");
    ...
    formEl.submit(function(){
        ...
        if( appendAddressBlockEl.is(":visible") ){
            if( $.trim(nameInputEl.val()) == "" ){
                result = false;
                nameBorderEl.addClass("warn");
            }else{
            ...
            if( $.trim(cityInputEl.val()) == "" ){
                ...
                cityBorderEl.removeClass("warn");
            }
            if( $.trim(addressInputEl.val()) == "" ){
                result = false;
                addressBorderEl.addClass("warn");
            }else{
                addressBorderEl.removeClass("warn");
            }
            if( $.trim(phoneInputEl.val()) == "" ){
                result = false;
                phoneBorderEl.addClass("warn");
            }else{
                phoneBorderEl.removeClass("warn");
            }
            if( $.trim(zipInputEl.val()) == "" ){
                result = false;
                zipBorderEl.addClass("warn");
            }else{
                zipBorderEl.removeClass("warn");
            }
        }
        if( recommendIndex == 1){
            if( $.trim( recommendInputEl.val() ) == ""){
                result = false;
                recommendInputBlockEl.addClass("warn");
            }else{
                recommendInputBlockEl.removeClass("warn");
            }
        }
        if( !result ){
            html7BodyEl.animate({ "scrollTop": stepNavEl.offset().top });
        }
        return result;
    });
};
