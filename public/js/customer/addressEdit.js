...
(function(){
    ...
    this.setValue = function(val){
        var i,item;
        for( i=0; i < this.useds.length;i++){
            item = this.useds[i];
            if( val == item.text() ){
                item.prop("selected","selected");
                this.index = $.inArray(val, this.data );
...
var EBE_AddressSelector = function(){
    ...
    var selectEls = $(".comm_contentMainPanel select");
    ...
    comboxs[0].setValue( selectEls.eq(0).attr("defaultVal") );
    comboxs[1].setValue( selectEls.eq(1).attr("defaultVal") );
    comboxs[2].setValue( selectEls.eq(2).attr("defaultVal") );
};

var EBE_EditManager = function(){
    var formEl = $(".comm_contentMainPanel form");
    var borderEls = formEl.find(".inputText,.inputSelect");
    var inputEls = formEl.find("input[type='text'],select");

    formEl.submit(function(){
        var i,inpueEl,result = true;
        for(i=0;i< inputEls.length;i++){
            if( i==2 || i==4 | i==7 ){continue;}
            if( $.trim(inputEls.eq(i).val()) == "" ){
                result = false;
                borderEls.eq(i).addClass("comm_form_validationFailed");
            }else{
                borderEls.eq(i).removeClass("comm_form_validationFailed");
            }
        }
        return result;
    });
